...
void loop() {
  ...
  // Continuous physical notification when waiting for prompt answer
  static uint32_t lastPromptNotifyMs = 0;
  static bool promptFlashHigh = false;
  if (inPrompt) {
    // Beep every 2 seconds
    if (now - lastPromptNotifyMs > 2000) {
      lastPromptNotifyMs = now;
      beep(1200, 80);
    }
    // Flash screen brightness every 500ms
    if (!screenOff) {
      bool shouldBeHigh = ((now - promptArrivedMs) % 1000) < 500;
      if (shouldBeHigh != promptFlashHigh) {
        promptFlashHigh = shouldBeHigh;
        if (promptFlashHigh) M5.Lcd.setBrightness(255);
        else applyBrightness();
      }
    }
  } else {
    lastPromptNotifyMs = 0;
    if (promptFlashHigh) {
      promptFlashHigh = false;
      if (!screenOff) applyBrightness();
    }
  }
  ...
  if (M5.BtnA.pressedFor(600) && !btnALong && !swallowBtnA) {
    ...
    beep(800, 60);
    if (inPrompt) {
      // Hold A on a pending prompt = "Yes, and don't ask again" (option 2).
      // The release handler skips "once" because btnALong is now set.
      char cmd[96];
