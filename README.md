...
## Using without Desktop App (Claude Code via PTY Wrapper)

If you have Anthropic API credits but no Claude.ai subscription, you can use the Hardware Buddy directly with claude (Claude Code) by utilizing the included Python PTY wrapper script.

The wrapper intercepts Claude Code terminal output, recognizes prompt states, and bridges this information to the ESP32 device via Bluetooth. When you approve or deny a prompt on the physical device, it relays the y or n answer back to the Claude Code terminal.
...
pip install bleak pexpect
```

**Usage:**

Run Claude Code by prepending tools/claude_wrapper.py:

```bash
./tools/claude_wrapper.py claude
```

*(Note: If you run into pairing issues like Peer removed pairing information, you may need to open your OS Bluetooth settings and unpair/forget "Claude-XXXX" manually before running the script).*
